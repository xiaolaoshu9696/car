...
//添加头文件 @2019/02/09
#include "../User/ec_mpu6050.h"
#include "../User/ec_delay.h"
...
//定义使用的芯片，因为该文件适用于多个芯片（新增）
#define MPU6050
//定义 MSP430，由于该驱动库没有 STM32 的，所以定义 MSP430，然后把 430 相关的东西删除（新增）
#define MOTION_DRIVER_TARGET_MSP430

#if defined MOTION_DRIVER_TARGET_MSP430

//以下 4 行（删除） @2019/02/09
//-/#include "msp430.h"
...
//（新增）以下 4 行 @2019/02/09
#define i2c_write MPU6050_I2C_BufferWrite
#define i2c_read MPU6050_I2C_BufferRead
#define delay_ms MPU6050_delay_ms
#define get_ms MPU6050_get_ms

//以下 '//-/' 开始的行（删除） @2019/02/09
...
int mpu_init(struct int_param_s *int_param)
{
    ...
    {
        if (i2c_read(st.hw->addr, st.reg->prod_id, 1, &(data[0]) )) //修改
            return -1;
        rev = data[0] & 0x0F;
        if (!rev)
        {
